...
def boar_brawl(player_score, opponent_score):
    ...
    opponent_second = opponent_score // 10 % 10
    return max(1, abs(player_first - opponent_second) * 3)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(0, goal):
        for opponent_score in range(0, goal):
            if strategy(score, opponent_score) != target:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_num_rolls = 1
    max_score = average_roll_dice(1, dice)
    cur_num_rolls = 2
    while cur_num_rolls <= 10:
        cur_score = average_roll_dice(cur_num_rolls, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    roll_0 = sus_points(score + boar_brawl(score, opponent_score))
    roll_num_rolls = sus_points(score + threshold)
    if roll_0 >= roll_num_rolls:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
